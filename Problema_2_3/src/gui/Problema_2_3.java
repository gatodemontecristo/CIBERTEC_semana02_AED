...
public class Problema_2_3 extends JFrame implements ActionListener {
			...
			try {
				...
				try {
					...
					try {
						...
						// Seteo : Incremento 20% Precio
						e.setPrecioDolar(e.getPrecioDolar() + (e.getPrecioDolar() * 0.20));
						listado(e);
						limpieza();
					}
					catch (Exception e) {
	...
	void listado(Edificio x){
		...
		imprimir("Nro Dptos\t\t: " + x.getNdepa());
		imprimir("Nro. Pisosv\t\t: " + x.getCantpiso());
		imprimir("Precio Dolar\t\t: " + x.getPrecioDolar());
		imprimir("Precio Dolar Edificio\t: " + x.calcularPrecioEdificio());
		imprimir("");
	}
