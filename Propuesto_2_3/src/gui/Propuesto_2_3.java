...
public class Propuesto_2_3 extends JFrame implements ActionListener {
			...
			if (nombre.length() > 0)
				try {
					...
					try {
						...
						limpieza();
						Empleado e = new Empleado(codigo,celular , nombre, sueldo);
						listado(e);
						
						e.setNum_celular(999888777);
						e.setSueldo_soles(e.getSueldo_soles()+200);
						listado(e);
	...
	void listado(Empleado x){
		...
		imprimir("Celular\t\t: " + x.getNum_celular());
		imprimir("Sueldo soles\t: " + x.getSueldo_soles());
		imprimir("Tipo sueldo\t: " + x.tipoSueldo());
		imprimir("");
	}
